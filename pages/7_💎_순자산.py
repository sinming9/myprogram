...
저장키 = "networth"
기본설정 = {
    # 자산표 형식 표시. 2 = 소유자 태그가 붙은 표로 이미 옮긴 자료.
    #  ★ 기본값은 반드시 0 이어야 합니다.
    #    여기에 2 를 두면 아직 옮기지 않은 자료도 '이미 옮긴 것' 으로 보고
    #    아래 변환을 건너뜁니다. 그러면 예전에 저장한 배우자 자산·기타자산이
    #    표에 안 들어오고, 자동 연동값(투자·연금·대출)도 비어 버립니다.
    #    저장할 때 2 로 올려 두면, 표를 전부 비운 뒤 다시 열어도 예전 값이
    #    되살아나지 않습니다.
    "형식": 0,
    "자산표": [],
    "기록": [],
...
def 자동_수집():
    """다른 페이지에 저장된 자료로 값을 모읍니다 (네트워크 없이).

    투자자산과 연금자산을 **나눠서** 가져옵니다. 연금저축·퇴직연금·IRP 는
    만 55세 전에 뺄 수 없어서 성격이 전혀 다르고, 순자산 표에서도 '투자' 가
    아니라 '연금' 대분류로 들어가야 맞습니다.
    """
    결과 = {"대출잔액": 0.0, "부동산공시": 0.0,
          "투자자산": 0.0, "연금자산": 0.0, "메모": []}
    ...
    # ---- 자산배분: 계좌 성격에 따라 투자 / 연금으로 갈라 담습니다 ----
    #  ※ 예전에는 '현재가(수동)' 이나 '평가액(직접입력)' 이 있는 종목만 셌습니다.
    #    시세 조회로 쓰시는 분은 그 칸이 비어 있어서 투자자산이 0원으로
    #    나왔습니다. 이제 평균단가까지 내려가며 어림합니다.
    자료V = storage.불러오기("portfolio", None)
    if 자료V and 자료V.get("종목"):
        from engines import portfolio as PF
        계좌별 = PF.오프라인_계좌별(자료V["종목"])
        투자 = sum(v for k, v in 계좌별.items() if PF.인출등급(k) != "잠김")
        연금 = sum(v for k, v in 계좌별.items() if PF.인출등급(k) == "잠김")
        시세필요 = [x for x in 자료V["종목"]
                 if not x.get("평가액(직접입력)") and not x.get("현재가(수동)")]
        결과["투자자산"] = 투자
        결과["연금자산"] = 연금
        if 투자 or 연금:
            꼬리 = (f" · {len(시세필요)}/{len(자료V['종목'])}종목은 평균단가로 어림"
                  if 시세필요 else "")
            if 투자:
                결과["메모"].append(f"투자자산 ← 📊 자산배분{꼬리}")
            if 연금:
                결과["메모"].append("연금자산 ← 📊 자산배분 (연금계좌 합계)")

    # ---- 연금 페이지에 직접 넣은 잔액이 있으면 그것을 씁니다 ----
    #  연금 페이지는 자산배분 값을 미리 채워 주고 사용자가 고칠 수 있게 합니다.
    #  거기서 손댄 값이 더 정확하니(원리금보장 상품처럼 종목 목록에 없는 것도
    #  포함) 있으면 그쪽을 우선합니다. 두 값을 더하면 이중 계상이 됩니다.
    자료N = storage.불러오기("pension", None)
    if 자료N:
        연금_직접 = (float(자료N.get("퇴직_잔액") or 0)
                 + float(자료N.get("저축_잔액") or 0))
        if 연금_직접 > 0:
            결과["연금자산"] = 연금_직접
            결과["메모"] = [m for m in 결과["메모"] if not m.startswith("연금자산")]
            결과["메모"].append("연금자산 ← 🧓 연금 (직접 넣은 잔액)")
    return 결과
