...
편집표 = st.data_editor(
    ...
    column_config={
        ...
        "내 지분(%)": st.column_config.NumberColumn(
            ...
            help="'공동' 일 때만 씁니다. **0 을 적으면 전부 가족 몫**이 되고, "
                 "아예 비우면 반반(50%)으로 봅니다. "
                 "전부 가족 몫이라면 소유자를 '배우자' 로 두는 편이 낫습니다."),
...
for _, row in 편집표.iterrows():
    만기 = 날짜로(row.get("만기일"))
    # 지분은 '빈 칸' 과 '0' 을 구별해서 넘겨야 합니다.
    #  빈 칸을 100 으로 채워버리면 "전부 가족 몫(0%)" 을 적을 방법이 없어집니다.
    지분 = row.get("내 지분(%)")
    지분 = None if NW.지분_비었나(지분) else 숫자로(지분, 0.0)
    새행들.append({
...
# 자산과 부채의 지분이 짝이 안 맞으면 알려줍니다.
#  (집은 내 몫 60% 인데 그 집 대출은 0% 로 둔 경우 등)
어긋남 = NW.지분_어긋남(집계)
if 어긋남:
    if 어긋남["차이"] > 0:
        st.warning(
            f"**부동산과 대출의 지분이 어긋납니다.** 부동산은 내 몫이 "
            f"{어긋남['부동산비중']:.0f}% 인데 부채는 {어긋남['부채비중']:.0f}% 로 "
            "두셨습니다. 이러면 **대출 없는 집을 그만큼 가진 셈**이 되어 "
            "내 몫이 실제보다 크게 나옵니다.\n\n"
            "집에 걸린 대출이라면 집과 **같은 지분**을 넣으세요. 명의가 정말 "
            "배우자 단독이라면 집도 대출도 소유자를 `배우자` 로 두는 것이 맞습니다.",
            icon="⚖️")
    else:
        st.warning(
            f"**부동산과 대출의 지분이 어긋납니다.** 부채는 내 몫이 "
            f"{어긋남['부채비중']:.0f}% 인데 부동산은 {어긋남['부동산비중']:.0f}% 로 "
            "두셨습니다. 빚만 내가 지고 집은 가족 것으로 잡혀서 내 몫이 "
            "실제보다 작게 나옵니다.", icon="⚖️")
...
with 탭내몫:
    ...
    with st.expander("ℹ️ 소유자를 어떻게 정하나요"):
        st.markdown(
            ...
            "- **배우자** — 배우자 명의. 가구에만 들어가고 내 몫은 0입니다.\n"
            "- **공동** — 공동명의. **내 지분(%)** 만큼만 내 몫이 됩니다. "
            "등기 지분이 6:4 면 60 을 넣으세요.\n\n"
            "**지분 칸의 0 과 빈 칸은 뜻이 다릅니다.**\n"
            "- `0` → 전부 가족 몫 (내 몫 0원)\n"
            "- 빈 칸 → 반반(50%)\n\n"
            "전부 가족 몫으로 둘 생각이라면 `공동 + 0` 보다 소유자를 "
            "**`배우자`** 로 두는 편이 뜻이 분명합니다. 결과는 같습니다.")
        st.info(
            "**대출은 그 자산과 같은 지분으로 두세요.** 집을 공동 60% 로 "
            "두고 그 집에 걸린 주택담보대출만 가족 몫(0%)으로 빼면, "
            "**대출 없는 집을 60% 가진 셈**이 되어 내 몫이 부풀려집니다.\n\n"
            "명의가 정말 배우자 단독이라면 집도 대출도 `배우자` 로 두는 것이 "
            "맞습니다. 그러면 둘 다 가구에만 잡히고 짝이 맞습니다.", icon="⚖️")
