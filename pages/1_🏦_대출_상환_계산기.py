...
# '계산하기' 는 한 번 누르면 계속 보이는 방식이어야 합니다.
#  ★ st.button 은 **누른 직후 딱 한 번만** True 입니다.
#    예전에는 그 값을 그대로 문지기로 썼습니다(if not 계산클릭: st.stop()).
#    그래서 결과 화면 안의 아무 칸(통장 확인일, 표 보기, 펼치기 …)을
#    건드리면 화면이 다시 그려지면서 계산클릭이 False 가 되고,
#    결과가 통째로 사라졌습니다. 다시 '계산하기' 를 눌러야 했습니다.
#
#    누른 사실을 세션에 남겨 두면 이후 리런에도 결과가 유지됩니다.
#    입력값을 바꾸면 어차피 매번 다시 계산되니 값은 항상 최신입니다.
if st.button("📊 계산하기", type="primary", width="stretch"):
    st.session_state["대출_계산함"] = True

# ==========================================================================
# 결과
# ==========================================================================
if not st.session_state.get("대출_계산함"):
...
with 탭요약:
    ...
    if not 진행["시작함"]:
        ...
        st.progress(min(max(진행["진행률"] / 100, 0.0), 1.0),
                    text=f"회차 {진행['낸회차']}/{진행['전체회차']} "
                         f"({진행['진행률']:.0f}%)")
        st.progress(min(max(진행["원금진행률"] / 100, 0.0), 1.0),
