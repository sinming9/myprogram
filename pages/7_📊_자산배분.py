...
if not st.session_state.get("_자산_자동로드"):
    ...
    if 저장된 and isinstance(저장된, dict) and 저장된.get("종목"):
        ...
        if 저장된.get("목표기준"):
            st.session_state["자산_목표기준"] = 저장된["목표기준"]
        if 저장된.get("조정계좌"):
...
def _설정_적용(데이터):
    if isinstance(데이터, dict) and 데이터.get("종목") is not None:
        ...
        if 데이터.get("목표기준"):
            st.session_state["자산_목표기준"] = 데이터["목표기준"]
        if 데이터.get("조정계좌"):
...
with 탭조정:
    ...
    기본선택 = [c for c in 있는계좌 if c not in PF.조정_제약계좌] or 있는계좌
    조정계좌 = st.multiselect(
        "리밸런싱 대상 계좌", 있는계좌,
        default=st.session_state.get("자산_조정계좌") or 기본선택,
        key="자산_조정계좌",
    ...
    대상종목 = [x for x in 종목들 if x.get("계좌") in 조정계좌]
    if not 대상종목:
        st.info("리밸런싱 대상 계좌를 하나 이상 골라주세요.", icon="📝")
        대상종목 = 종목들
...
저장_불러오기(저장키,
          {"종목": 읽은목록,
           ...
           #   숫자와 화면 숫자가 서로 달라집니다.
           "목표기준": st.session_state.get("자산_목표기준", "자산군"),
