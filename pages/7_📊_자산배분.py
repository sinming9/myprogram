...
for 숫자열 in ("수량", "평균단가", "현재가(수동)", "주당배당(수동)"):
...
종목df = st.data_editor(
    ...
    column_config={
        ...
        "티커": st.column_config.TextColumn(help="한국은 6자리 숫자(005930), 미국은 심볼(AAPL)"),
        ...
        "현재가(수동)": st.column_config.NumberColumn(
            min_value=0.0, help="자동 조회가 안 되는 종목만 넣으세요"),
...
c1.caption(f"종목 {len(읽은목록)}개 · 시세는 30분간 기억합니다")
...
if 실패목록:
    with st.expander(f"⚠️ 시세 조회 실패 {len(실패목록)}건 — 평균단가로 계산했습니다"):
        for m in 실패목록:
            st.text("  " + m)
        st.caption("티커를 확인하시거나, 표의 **현재가(수동)** 칸에 직접 넣으세요.")
...
종목표 = pd.DataFrame([{
    ...
    "연배당": round(s["연배당"]),
    "배당률(%)": round(s["배당수익률"], 2),
    "52주 위치(%)": None if s["52주위치"] is None else round(s["52주위치"]),
} for s in 정렬])
st.dataframe(종목표.style.format({"평가액": "{:,}", "연배당": "{:,}"}, na_rep="-"),
