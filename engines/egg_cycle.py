...
# ==========================================================================
# 국면별 자산군 방향  (자산배분 페이지와 연동)
# ==========================================================================
# [중요] 달걀 모형은 '방향'만 말합니다. 구체적인 비중(%)은 제시하지 않습니다.
#       그래서 여기서도 ↑ / ↓ / — 만 정하고, 목표 비중은 사용자가 정합니다.
#       "국면 D→E 니까 해외주식 50%" 같은 숫자를 만들어내면 근거 없는
#       정밀함이 됩니다.
#
# 모형이 다루는 자산은 주식·채권·예금·부동산 네 가지입니다.
# 암호화폐·원자재는 모형 범위 밖이라 판단하지 않습니다(None).
_주식, _채권, _현금, _부동산 = "국내주식", "채권", "현금성", "리츠"

국면_방향 = {
    "D":   {"국내주식": "↑", "해외주식": "↑", "채권": "↓", "현금성": "↓", "리츠": "↓"},
    "D-E": {"국내주식": "↑", "해외주식": "↑", "채권": "↓", "현금성": "↓", "리츠": "↓"},
    "E":   {"국내주식": "↑", "해외주식": "↑", "채권": "↓", "현금성": "↓", "리츠": "↓"},
    "E-F": {"국내주식": "—", "해외주식": "—", "채권": "—", "현금성": "↑", "리츠": "↓"},
    "F":   {"국내주식": "↓", "해외주식": "↓", "채권": "↑", "현금성": "↑", "리츠": "↓"},
    "F-A": {"국내주식": "↓", "해외주식": "↓", "채권": "↑", "현금성": "↑", "리츠": "—"},
    "A":   {"국내주식": "↓", "해외주식": "↓", "채권": "↑", "현금성": "↓", "리츠": "—"},
    "A-B": {"국내주식": "↓", "해외주식": "↓", "채권": "↑", "현금성": "↓", "리츠": "—"},
    "B":   {"국내주식": "—", "해외주식": "—", "채권": "↑", "현금성": "↓", "리츠": "↑"},
    "B-C": {"국내주식": "—", "해외주식": "—", "채권": "—", "현금성": "↓", "리츠": "↑"},
    "C":   {"국내주식": "—", "해외주식": "—", "채권": "↓", "현금성": "↓", "리츠": "↑"},
    "C-D": {"국내주식": "↑", "해외주식": "↑", "채권": "↓", "현금성": "↓", "리츠": "↑"},
}
...
def 국면_자산방향(국면코드: str) -> dict:
    """그 국면에서 모형이 시사하는 자산군별 방향. 모르면 빈 dict."""
    return dict(국면_방향.get(국면코드, {}))


def 배분_대조(국면코드: str, 현재비중: dict, 목표비중: dict = None) -> list:
    """모형의 방향과 내 배분을 나란히 놓습니다.

    현재비중 = {"해외주식": 61.2, "채권": 20.0, ...}
    목표비중 = 사용자가 정한 목표(있으면). 없으면 방향만 보여줍니다.

    ※ 모형은 방향만 말하므로 '얼마나' 는 스스로 정해야 합니다.
      목표를 넣으면 괴리 방향이 모형과 같은지 다른지 표시합니다.
    """
    방향 = 국면_자산방향(국면코드)
    이름들 = sorted(set(방향) | set(현재비중) | set(목표비중 or {}))
    출력 = []
    for 이름 in 이름들:
        현재 = float((현재비중 or {}).get(이름, 0.0))
        목표 = (목표비중 or {}).get(이름)
        d = 방향.get(이름)
        괴리 = (현재 - float(목표)) if 목표 not in (None, "") else None

        일치 = None
        if d and 괴리 is not None and d != "—":
            # 모형이 확대(↑)라는데 이미 목표보다 많으면(괴리>0) 방향이 어긋남
            일치 = (괴리 < 0) if d == "↑" else (괴리 > 0)

        출력.append({
            "자산군": 이름,
            "현재비중": 현재,
            "목표비중": None if 목표 in (None, "") else float(목표),
            "괴리": 괴리,
            "모형방향": d if 이름 not in 모형_범위밖 else None,
            "모형설명": 방향_설명.get(d, "") if 이름 not in 모형_범위밖 else "모형 범위 밖",
            "방향일치": 일치,
        })
    return sorted(출력, key=lambda x: -x["현재비중"])
