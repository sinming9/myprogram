...
def 호텔_검색(행, 키, 호스트=기본_호스트, 통화=기본_통화):
    ...
    매개 = {
        "dest_id": 정리["dest_id"],
        "search_type": 정리["종류"] if 정리.get("종류") else "CITY",
...
def 알림_문장_짧게(정리, 현재가, 판정, 요약=None) -> str:
    """카카오용 짧은 글 (200자 한도).

    긴 글을 잘라내는 것보다 처음부터 짧게 쓰는 편이 낫습니다. 자르면
    "지금까지 최저" 같은 뒷줄이 먼저 사라지는데, 그게 정작 중요합니다.
    """
    값 = _수(현재가, 0)
    줄 = [f"🏨 {정리['이름']}"]
    들, 나 = 정리["체크인"], 정리["체크아웃"]
    if 들 and 나:
        줄.append(f"{들.strftime('%m/%d')}~{나.strftime('%m/%d')}"
                 f" {정리['밤수']}박 · {정리['인원글']}")
    if 정리["밤수"] > 0:
        줄.append(f"💰 {값:,.0f}원 (1박 {값 / 정리['밤수']:,.0f})")
    else:
        줄.append(f"💰 {값:,.0f}원")
    if (판정 or {}).get("목표달성"):
        줄.append(f"✅ 목표 {_수(정리.get('목표가'), 0):,.0f}원 이하")
    if (판정 or {}).get("하락달성"):
        줄.append(f"✅ 처음보다 {판정['하락폭']:.1f}% 내림")
    if 요약 and 요약.get("횟수", 0) > 1:
        if 값 <= 요약["최저"]:
            줄.append("🔥 지금까지 최저")
        else:
            줄.append(f"📉 최저 {요약['최저']:,.0f}원")
    return "\n".join(줄)
...
# 알림 채널 — 텔레그램 · 카카오
# ==========================================================================
#  두 채널을 함께 씁니다. 한쪽이 실패해도 다른 쪽은 가야 합니다.
#  특히 카카오는 리프레시 토큰이 2개월마다 끊길 수 있어서, 텔레그램을
#  받침대로 두면 알림이 조용히 멈추는 일을 막을 수 있습니다.
...
def 알림_보내기(채널, 긴글, 짧은글=None) -> tuple:
    ...
    없는 채널은 넣지 않거나 빈 값으로 두면 건너뜁니다.

    결과들 = [(채널이름, 성공여부, 메시지), …]

    ★ 새_카카오_리프레시 가 None 이 아니면 **꼭 저장해야 합니다.**
      카카오가 리프레시 토큰을 갱신해 준 것이고, 저장하지 않으면
      다음에 못 씁니다.
    """
    채널 = 채널 or {}
    결과들 = []
    ...
    카 = 채널.get("카카오") or {}
    if str(카.get("rest키") or "").strip() and str(카.get("리프레시")
                                                 or "").strip():
        try:
            from engines import kakao as KK
        except ImportError:
            결과들.append(("카카오", False,
                        "engines/kakao.py 가 없습니다. 함께 올리세요."))
        else:
            좋음, 말, 새것 = KK.갱신하고_보내기(
                카["rest키"], 카["리프레시"], 짧은글 or 긴글,
                카.get("시크릿"), 카.get("링크"))
            결과들.append(("카카오", 좋음, 말))
            보낸수 += 1 if 좋음 else 0
            if 새것:
